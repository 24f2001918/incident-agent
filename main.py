...
def build_otlp(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # SERVER root — no status (matches reference exactly).
    root_span = {
    ...
    if state.get("incoming_span_id"):
        root_span["parentSpanId"] = state["incoming_span_id"]
    ...
    def emit_action(act: Dict[str, Any]):
        ...
        for att in act["attempts"]:
            ...
            if att.get("receiptId") is not None:
                attrs.append(_attr("ga5.receipt.id", att["receiptId"]))
            if att.get("nonce") is not None:
    ...
    if eff and eff.get("needs_approval"):
        ...
        if eff.get("approvalNonce") is not None:
...
def _confirm_attempt(state: Dict[str, Any], act: Dict[str, Any],
    ...
    if not att:
        return
    receipt_id = f"rcpt_{_hexid(state['runId'] + ':' + act['actionId'] + ':receipt', 16)}"
    hx = _hexid(state["runId"] + ":" + act["actionId"] + ":nonce", 32)
    nonce = f"{hx[:8]}-{hx[8:12]}-{hx[12:16]}-{hx[16:20]}-{hx[20:32]}"
    att["status"] = 200
...
def _handle_outcomes(state: Dict[str, Any], receipt_id: str,
    ...
    for oc in outcomes or []:
        ...
        if rclass is not None:
            r_entry["resultClass"] = rclass
        if nonce is not None:
            r_entry["nonce"] = nonce
        state["receiptLog"].append(r_entry)
...
def _handle_approvals(state: Dict[str, Any], receipt_id: str,
    ...
    for ap in approvals or []:
        ...
        if nonce is not None:
            a_entry["nonce"] = nonce
        state["receiptLog"].append(a_entry)
